...
def gen_embed(sentence, type_embed = 'spacy', tag = None):
    if type_embed == 'spacy':
        ...
        # Convert the vectorized sentence into a dense matrix
        dense_matrix = vectorized_sentence.todense()

        # Calculate the tf-idf word embeddings
        tfidf_embedding = []
        for feature_name, index in feature_names.items():
            tf_idf_value = dense_matrix[0, index]
            tfidf_embedding.append(tf_idf_value)
        # Print the tf-idf embeddings
        embed =  np.array(tfidf_embedding)
        return embed
